...
class ReliefDataIndex extends Component {
  ...
  findByStatus(){
    ...
    firebase.firestore().collection("family_data").where("relief_status", "==", set.state.relief_status).where("relief_type", "==", 'goods')
    .get()
    .then((querySnapshot) => {
      const list = [];
      querySnapshot.forEach((doc) => {
                          ...
                          this.state.list.map((single, index) =>
                            <tr key={index}>
                              ...
                              this.state.hide_input === false ? 
                                <>
                                  <td> 
                                    <Button className="btn-block my-4 btn-sm" color="info" type="button">Quarantined</Button>
                                  </td>
                                </> : <></> 
                              }
                              <td>
                                {
                                  ...
                                  <>
                                  {
                                    ...
                                    <>
                                    <Button 
                                      onClick={() => this.openSMSPopup([single.contact_number])}
                                      disabled={ this.state.selected_gnd === undefined }
